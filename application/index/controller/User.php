...
//        dump($loglist);die;
    ...
    public function blessings_transfer()
    {
        ...
        if ($this->request->isPost()) {
            ...
            //减少当前用户
            moneyLog($this->user_id,$tranUser['id'],'pay_points',-$number,8,'饲料转增');
            //增加转让用户
            moneyLog($tranUser['id'],$this->user_id,'pay_points',$number,8,'饲料转增');
            //累计充值
    ...
    public function profit()
    {
        $map = [];
        $map['user_id'] = $this->user_id;
        $map['currency'] = ['in', ['share_integral','team_integral','zc_integral']];
...
//        dump($payment);
    ...
    {
        $banklist = Db::name('user_payment')->where('uid',$this->user_id)->select();
        //dump($banklist);
        return view()->assign('banklist',$banklist);
    }
