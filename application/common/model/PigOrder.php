...
//hfh 0213
            ...
            $userPig['end_time'] = time()+$pigInfo['cycle']*24*3600-(int)$sell_end_time;
...
//            Db::name('user_pigs')->where($map)->setField('status',2);
            //奖励PIG
            //hfh 0214
            moneyLog($orderInfo['uid'],$orderInfo['sell_id'],'pig',$pigInfo['pig'],9,'买入奖励wia');

            //奖金记录
            addReward($orderInfo['uid'],$orderInfo['sell_id'],'pig',$pigInfo['pig'],5,'交易奖励wia');
            return true;
        } else{
            //$this->error('操作失败');
            return false;
