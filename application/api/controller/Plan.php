...
use think\Cache;
use think\Log;
use app\admin\controller\Zonghe;
/**
 * 计划任务
 * Class Index
...
class Plan extends Controller
{
   ...
   public function noYuYue(){
    ...
    public function autoSell(){
        ...
        $userPigs= DB::name('user_pigs')->where($m)->select();

        foreach ($userPigs as $pig){
            if($pig['end_time']<time()){
                $dd=[
                    'uid' => $pig['uid'],
                    'pig_id' => $pig['pig_id'],
                    'sell_id' => 0,
                    'price' => $pig['price']+$pig['contract_revenue'],
                    'create_time' => time(),
                    'status'=>0,
                    'pig_name'=>$pig['pig_name'],
                    'order_no'=> create_trade_no()
                ];
                $newOrderId = Db::name('PigOrder')->insertGetId($dd);
                $pig['order_id']=$newOrderId;
                Db::name('user_pigs')->where('id',$pig['id'])->setField([
                    'status'=>1,
                    'order_id'=>$newOrderId,
                    'end_time'=>time()
                ]);
